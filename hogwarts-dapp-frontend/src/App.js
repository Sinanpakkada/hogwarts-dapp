    ...
    if (started) {
	    if (window.ethereum) {
	      ...
	      window.ethereum.request({ method: 'eth_requestAccounts' }).then((accounts) => {
	        setAccount(accounts[0]);
					const hogwartsAddress = "0x4b2559a86D8F7F4fF226b83ddFFAb06FEec84696";
	        const randomHouseAddress = "0xec2EaC37Af25f2Ba6Cd7fE8cb217E7ffeFb3Ae21";
          console.log(accounts[0]);
          console.log(hogwartsAddress);
          console.log(randomHouseAddress);
        ...
        console.log('Hogwarts Contract Instance:', hogwartsInstance);
console.log('Random House Contract Instance:', randomHouseInstance);
  ...
  const connectMetamask = async () => {
    try {
      ...
      setConnected(true);
      console.log("metamask connected ?",setConnected);
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const checkMinted = async () => {
    await checkName();
    if (hogwartsContract) { // Null check before using hogwartsContract
        ...
        // Handle case where hogwartsContract is null
        console.log("hogwarts contract is null");
    }
};
